...
public class Puzzle {
    ...
    public boolean checkMove(int col, int row, int val){
        for (int i = 0; i<9; i++){
            if (values[col][i] == val) { return false; }
            if (values[i][row] == val) { return false; }
        }
        int colrem = col%3; int rowrem = row%3;
        if (colrem == 0){
            if (rowrem == 0){
                if (values[col+1][row+1] == val) { return false; }
                if (values[col+1][row+2] == val) { return false; }
                if (values[col+2][row+1] == val) { return false; }
                if (values[col+2][row+2] == val) { return false; }
            }
            if (rowrem == 1){
                if (values[col+1][row-1] == val) { return false; }
                if (values[col+1][row+1] == val) { return false; }
